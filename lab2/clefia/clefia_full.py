...
#
# Source GitHub repo - https://github.com/thomasxmeng/Clefia_cipher_python_implementation_for_general_testing/
#
# Date: 2012-02-03
#
...
class CipherWorker(object):
    ENCRYPT = 'ENCRYPT'
    DECRYPT = 'DECRYPT'
    modes = [ENCRYPT, DECRYPT]
    key_sizes = [16, 24, 32]

    def __init__(self, mode: str, key_file_name: str, source_file_name: str, key_size: int):
        if mode not in CipherWorker.modes:
            raise AttributeError(f'mode {mode} is invalid, choose one of '
                                 f'{CipherWorker.modes[0]} {CipherWorker.modes[1]}')
        self.mode = mode

        if not os.path.isfile(key_file_name):
            raise AttributeError(f'no key file found with name {key_file_name}')
        self.key_file_name = key_file_name

        if not os.path.isfile(source_file_name):
            raise AttributeError(f'no source file found with name {source_file_name}')
        self.source_file_name = source_file_name

        if key_size not in CipherWorker.key_sizes:
            raise AttributeError(f'invalid key size {key_size}, should be one of'
                                 f' {CipherWorker.key_sizes[0]} {CipherWorker.key_sizes[1]} {CipherWorker.key_sizes[2]}'
                                 )
    ...
    def parse_key(self, key_data):
        self.key = key_data[:self.key_size]

    def work(self) -> bytearray:
        # read key
        key_file = open(self.key_file_name, 'rb')
        src_file = open(self.source_file_name, 'rb')
        ...
        if len(key_data) < self.key_size:
            key_file.close()
            src_file.close()
            raise RuntimeError(f'to small key with size {len(key_data)} bytes, should be {self.key_size} bytes')

        setKey(int.from_bytes(key_data[: self.key_size], byteorder='little'), ksReverseTable[self.key_size])

        key_file.close()
        src_file.close()
        result = bytearray()

        iteration_count = len(src_file_data) // self.key_size

        if len(src_file_data) % self.key_size != 0:
            iteration_count += 1

        if self.mode == CipherWorker.ENCRYPT:
            work_function = encrypt
        elif self.mode == CipherWorker.DECRYPT:
            work_function = decrypt
        else:
            assert False

        for i in range(iteration_count):
            st_idx = i * self.key_size
            result += work_function(int.from_bytes(src_file_data[st_idx: st_idx + self.key_size], byteorder='little'))\
                .to_bytes(length=self.key_size, byteorder='little')
...
if __name__ == "__main__":
    ...
    if argc != 6:
        print(f"not enough arguments count - {argc - 1}, should be 4:"
              f"\n 'mode', 'key_file_name', 'source_file_name', 'output_file_name', 'key_size'", file=sys.stderr)
        sys.exit(1)
    _, mode, key_file_name, source_file_name, output_file_name, key_size = args
    key_size = int(key_size)

    try:
        cipher_worker = CipherWorker(mode, key_file_name, source_file_name, key_size)
    except AttributeError as e:
        print(e, file=sys.stderr)
        sys.exit(1)
    try:
        result = cipher_worker.work()
